...
class DummyCard:
    def __init__(self):
        ...
        # FIX: Add missing attributes required by SRS service
        self.lapses = 0
        self.is_leech = False
...
def test_srs_quality_0_resets():
    ...
    assert c.interval == 1
    assert c.lapses == 1  # Lapses should increment on failure
...
def test_srs_repeated_reviews():
    ...
    # First review
    update_card_after_review(c, 5)  # r=1, i=1
    
    # Second review
    update_card_after_review(c, 5)  # r=2, i=6 (standard SM-2)
    
    # Third review
    update_card_after_review(c, 5)  # r=3, i > 6
