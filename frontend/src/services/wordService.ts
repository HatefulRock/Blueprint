...
export const addWord = async (wordData: Omit<Word, 'familiarityScore' | 'language'>, targetLanguage: string): Promise<Word> => {
  try {
...
export const updateFamiliarity = async (term: string, change: 1 | -1, targetLanguage: string): Promise<Word> => {
  try {
    return await apiRequest<Word>(`/words/${encodeURIComponent(term)}/familiarity?language=${targetLanguage}`, {
      method: 'PUT',
      body: JSON.stringify({ change })
    });
  } catch (error) {
    // console.debug("Backend unreachable, updating locally.");
    const wordIndex = db.words.findIndex(w => w.term === term && w.language === targetLanguage);
    if (wordIndex !== -1) {
...
export const importDeck = async (file: File, name: string, language: string): Promise<{ deck: Deck, importedCount: number }> => {
    try {
...
export const createDeck = async (name: string, language: string): Promise<Deck> => {
    try {
